...
func GetAllrank(ctx *gin.Context) {
	...
	var respCovers []respModel.HotResp
	if tempdata != "" {
		data := []byte(tempdata)
	...
	//读取数据库
	likes, err := dao.GetCoversByLikes()
	if err != nil {
...
func GetDailyrank(ctx *gin.Context) {
	...
	// 	if err == nil {
	// 		//更新点赞确认即可
	// 		for _, item := range respCovers {
	// 			bo, err := dao.PackageCheckMysql(UserId, "cover", item.Cover_Id)
	// 			if err != nil {
	// 				log.Printf(err.Error())
	// 				// respCovers[i].Check = 0
	// 			} else if bo {
	...
	//没有缓存，读取数据库
	likes, err := dao.GetCoversByDate(date)
